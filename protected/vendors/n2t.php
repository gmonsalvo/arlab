...
class n2t
{
	...
	public function setSubunidad($psubunidad){
		$this->subunidad = $psubunidad;
	}

	public function setSeparador($pseparador){
	...
	private function centmillon($numero){
		if ($numero == 100000000)
			$this->num_letracmm = "cien millones";
		if ($numero > 100000000 && $numero <200000000){
			$this->flag=1;
			$this->num_letracmm = $this->centena($numero/1000000)." millones ".$this->cienmiles($numero%1000000);
		}
		if ($numero >= 200000000 && $numero <1000000000){
			$this->$flag=1;
			$this->num_letracmm = $this->centena($numero/1000000)." milllones ".$this->millon($numero%1000000);
		}
		if ($numero < 100000000)
			$this->num_letracmm = $this->millon($numero);
		return $this->num_letracmm;
	}

	public function convertirLetras($numero){
		$this->num_letras = $numero;
		$pos = strrpos($numero, ".");
		...
		if ($pos != -1){
			$dec = substr($numero,$pos+1);
			if ($dec == 0)
				$decimales = " ".$this->separador." cero ".$this->subunidad;
			else
				$decimales = " ".$this->separador." ".$this->centmillon($dec)." ".$this->subunidad;
			$ent = substr($numero,0,$pos);
		}
		else{
			$ent = (int) $numero;
		}
		...
		if ($ent == 0)
			$enteros = "cero ".$this->unidad;
		else
			$enteros = $this->centmillon($ent)." ".$this->unidad;

		$this->num_letras = $enteros.$decimales;

		// $num_letras = Double.toString($numero);
...
/*$converti = new n2t();
$pos = strrpos("200.0", ".");
echo $converti->convertirLetras("41230.50");
echo $pos;
*/
